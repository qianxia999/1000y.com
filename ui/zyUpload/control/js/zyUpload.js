		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">拖放文件到此处</span>';
					html += '			</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">继续选择文件</div>';
		            html += '					<div class="upload_btn">开始上传2</div>';
		            html += '				</div>';
		            html += '			</div>';
...
//					return arrFiles;
//				}else{
					var typearray=["image/bmp","image/jpeg","image/png","image/gif"];
					//var zhi=typearray.indexOf(file.type); //alert(zhi);
					for (var i = 0, file; file = files[i]; i++) {
						console.info(file);
						if (file.size >= 51200000) {
							alert('您这个"'+ file.name +'"文件大小过大');	
						//}else if(file.type != "image/jpeg") {
						}else if(typearray.indexOf(file.type)<0) {//by wind
...
//				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" class="imgBox">';//去掉A链接中的 href="#" by wind
			...
			this.createCorePlug = function(){
				var params = {
					...
					onFailure: function(file, response) {
						...
						$("#uploadInf").append("<p>上传失败！" + file.name + "</p>");	
